...
# Traditional message commands (work immediately)
@bot.command(name='ping')
async def ping_command(ctx):
    """Traditional ping command - works immediately"""
    latency = round(bot.latency * 1000)
    await ctx.send(f"🏓 Pong! Latency: {latency}ms")
...
async def status_command(ctx):
    ...
    try:
        ...
        embed.set_footer(
            text=f"Status checked at {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')} • Use !rally for calculator"
        )
...
async def help_command(ctx):
    ...
    embed = discord.Embed(
        title="🤖 Bot Commands",
        description="Available commands for the Bear Hunt Rally Calculator",
        color=0x00ff99
    )
    ...
    embed.add_field(
        name="💡 Slash Commands (when available)",
        value="• `/rally_calculator` - Same as !rally\n• `/status` - Same as !status\n• `/ping` - Same as !ping",
        inline=False
    )
